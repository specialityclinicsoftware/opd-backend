...
/**
   ...
   */
export const createVisit = async (visitData: Partial<IVisit> ): Promise<{
  ...
  visit: IVisitDocument | null;
    ...
    try {
      ...
      logger.info(`Visit created for patient: ${visitData.patientId}, Status: ${visit.status}`);
...
export const updatePreConsultation = async (
    ...
    try {
      ...
      if (visit.status !== VisitStatus.WITH_NURSE && visit.status !== VisitStatus.PENDING) {
        throw new Error(`Cannot update pre-consultation. Visit is in ${visit.status} status`);
      }
...
export const updateConsultation = async (
    ...
    try {
      ...
      if (visit.status !== VisitStatus.WITH_DOCTOR && visit.status !== VisitStatus.READY_FOR_DOCTOR) {
        throw new Error(`Cannot update consultation. Visit is in ${visit.status} status`);
      }
      ...
      const allowedFields = canEditNurseData ? [...doctorFields, ...nurseFields] : doctorFields;
...
/**
   ...
   */
export const getPatientVisits = async (patientId: string):Promise<IVisitDocument[]> =>  {
    try {
      ...
      logger.error(`Error in getPatientVisits service for patient ${patientId}:`, error);
      throw error;
    }
}
...
/**
   ...
   */
export const getVisitById = async (visitId: string):Promise<IVisitDocument | null> =>  {
    try {
      const visit = await Visit.findById(visitId)
...
/**
   ...
   */
export const getLatestVisit = async (patientId: string):Promise<IVisitDocument | null>  => {
    try {
      ...
      logger.error(`Error in getLatestVisit service for patient ${patientId}:`, error);
      throw error;
    }
}
...
/**
   * Cancel visit
   */
export const cancelVisit = async (visitId: string, reason?: string):Promise<IVisitDocument> =>  {
    try {
      const visit = await Visit.findById(visitId);
      ...
      logger.info(`Visit cancelled: ${visitId}, Reason: ${reason || 'Not specified'}`);
...
/**
   ...
   */
export const deleteVisit = async (visitId: string):Promise<IVisitDocument | null> =>  {
    try {
...
export const getPatientHistory = async (
    ...
    try {
      ...
      logger.error(`Error in getPatientHistory service for patient ${patientId}:`, error);
      throw error;
    }
}
